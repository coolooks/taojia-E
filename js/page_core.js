$(function(){
    $('.nav_item').hover(function(){ $(this).addClass('nav_item_on');},function(){$(this).removeClass('nav_item_on');});
	/*-----套餐和帮助 下拉列表------*/
    ...
    function yyUI(){
		$('body').append('<div class="mask"></div>');
		$('body').append('<div class="yy"><p>预约现场看家居</p><div><form><span><input type="text" name="name" placeholder="您的姓名" /></span><span><input type="text" name="tel" placeholder="您的电话" /></span><span><input type="text" name="tel" placeholder="您的小区（仅限洛阳地区）" /></span><input type="button" name="yy_btn" value="立即预约" /></form></div><div class="close"></div></div>');
	}
    ...
    $('.btnBox a#yuyue').on('click',function(){
		yyUI();
	})
    ...
    $('body').on('click','.yy .close',function(){
		$('.mask,.yy').remove();
	})
